...
def create_type_column(df):
    ...
    def seating_group(seats):
        try:
            ...
            return 'Unknown'  # If seating can't be con   verted to integer, return 'Unknown'
        if seats <= 4:
            return 'Small'
        elif 5 <= seats <= 6:
            return 'Medium'
...
def recommend_cars_by_type(
    ...
    require_better=True,
    top_n=3  # Number of cars to return
):
...
def recommend_cars_with_type_strategy(df, selected_car):
    ...
    first_suggestion = recommend_cars_by_type(
        df_copy, selected_car, seating_capacity_filter='gte', top_n=3
    )
    ...
    second_suggestion = recommend_cars_by_type(
        df_copy, selected_car, seating_capacity_filter='any', top_n=2
    )
    ...
    third_suggestion = recommend_cars_by_type(
        df_copy, selected_car, seating_capacity_filter='any', require_better=False, top_n=2
    )
...
def recommend_cars_by_price(
    df, selected_car, price_tolerance_percent=5, top_n=5
):
    selected_price = selected_car['Price_numeric']
    selected_feature_score = selected_car.get('Feature_score', 0)
...
def recommend_cars_by_same_variant(
    df, selected_car, price_tolerance_percent=5, top_n=5, yes_no_cols=None
):
    selected_price = selected_car['Price_numeric']
    selected_feature_score = selected_car.get('Feature_score', 0)
